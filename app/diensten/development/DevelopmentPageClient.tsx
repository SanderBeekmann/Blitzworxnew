...
function TypingTerminal() {
  ...
  const totalChars = codeLines.reduce((sum, line) => {
    ...
    return sum + lineChars + 1; // +1 for newline
  }, 0);
  ...
  useEffect(() => {
    ...
    const speed = 30 + Math.random() * 40; // variable typing speed
  ...
  const renderLines = () => {
    ...
    return codeLines.map((line, lineIdx) => {
      ...
      // Show cursor at the end of the current typing position
